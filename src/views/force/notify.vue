...
<script>
import { tasklist, edittask, addtask } from '@/api/force/notify'
  ...
  methods: {
    change1() {
      setTimeout(() => {
        this.temp.region1 == '2'
        this.$nextTick(() => {
          this.temp.region2 = this.temp.region1
        })
      }, 0)
    },
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.taskId = this.temp.taskId
          tempData.cityId = this.cityId

          tempData.title = this.temp.title
          tempData.createTime = this.temp.createTime
          tempData.releasePeople = this.temp.releasePeople
          tempData.recipient = this.temp.recipient

          this.disabledStatus = true
          edittask(tempData).then((response) => {
            if (response.code === 200) {
