          ...
          <template slot-scope="{row,$index}">
            ...
            >
              <div>
                <el-button
                  size="mini"
                  @click="$refs[`popover-` + row.educationId].doClose()"
                >取消</el-button>
                <el-button
                  type="primary"
                  size="mini"
                  @click="$refs[`popover-` + row.educationId].doClose();handleDelete(row)"
                >确定</el-button>
              </div>
              <el-button
                slot="reference"
...
export default {
  ...
  methods: {
    getList() {
      ...
      xczxeducationlist(params).then((response) => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addeducationinfo(this.temp).then((response) => {
            if (response.code === 200) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const tempData = {}
          tempData.educationId = this.temp.educationId
          ...
          tempData.addr = this.temp.addr
          this.disabledStatus = true
          editeducationinfo(tempData).then((response) => {
            if (response.code === 200) {
              this.disabledStatus = false
              this.dialogFormVisible = false
              this.$message({
    ...
    handleDelete(row) {
      deleducationinfo({ educationId: row.educationId }).then((response) => {
        if (response.code === 200) {
          this.$message({
            message: '删除成功',
            type: 'success',
