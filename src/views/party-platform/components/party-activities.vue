          ...
          <template slot-scope="{row,$index}">
            ...
            >
              <div>
                <el-button
                  size="mini"
                  @click="$refs[`popover-` + row.partyBuildId].doClose()"
                >取消</el-button>
                <el-button
                  type="primary"
                  size="mini"
                  @click="$refs[`popover-` + row.partyBuildId].doClose();handleDelete(row)"
                >确定</el-button>
              </div>
              <el-button
                slot="reference"
...
export default {
  ...
  methods: {
    getList() {
      ...
      partyinfolist(params).then((response) => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addpartyinfo(this.temp).then((response) => {
            if (response.code === 200) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const tempData = {}
          tempData.partyBuildId = this.temp.partyBuildId
          ...
          this.disabledStatus = true
          editpartyinfo(tempData).then((response) => {
            if (response.code === 200) {
              this.disabledStatus = false
              this.dialogFormVisible = false
              this.$message({
    ...
    handleDelete(row) {
      delpartyinfo({ partyBuildId: row.partyBuildId }).then((response) => {
